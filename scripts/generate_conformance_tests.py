...
def source_position(input_html: str, offset: int) -> Tuple[int, int]:
    """Return the one-based line and column at a raw input offset."""
    line = 1
    column = 1
    i = 0
    while i < offset:
        if input_html[i] == '\r':
            if i + 1 < offset and input_html[i + 1] == '\n':
                i += 1
            line += 1
            column = 1
        elif input_html[i] == '\n':
...
def normalize_processing_instruction_errors(input_html: str, errors: List) -> List:
    """Update legacy html5lib error expectations for current HTML PI rules."""
    if not input_html.startswith("<?"):
        return errors

    retained = [
        error for error in errors
        if error["code"] != "unexpected-question-mark-instead-of-tag-name"
    ]
    input_errors = [
        error for error in retained
        if error["code"] in (
            "control-character-in-input-stream",
            "noncharacter-in-input-stream",
            "surrogate-in-input-stream",
    ...
    state_errors = [error for error in retained if error not in input_errors]
    i = 2
    if i == len(input_html):
        line, col = source_position(input_html, i)
        return [{"code": "eof-in-processing-instruction", "line": line, "col": col}] + retained

    first = input_html[i]
    normalized_first = '\n' if first == '\r' else first
    if not (normalized_first.isascii() and (normalized_first.isalpha() or normalized_first == '_')):
        line, col = source_position(input_html, i)
        return input_errors + [{
    ...
    normalized = '\n' if input_html[i] == '\r' else input_html[i]
...
def error_code_variant(code: str) -> str:
    if code not in TOKENIZER_ERROR_CODES:
        raise ValueError(f"Unsupported tokenizer error code: {code}")
    return ''.join(part.capitalize() for part in code.split('-'))


def moonbit_error_column(input_html: str, line: int, column: int) -> int:
    """Convert html5lib UTF-16 columns to MoonBit Unicode-scalar columns."""
    current_line = 1
    current_column = 1
    surrogate_pairs = 0
    i = 0
    while i < len(input_html) and current_line <= line:
        if current_line == line and current_column >= column:
            break
        c = input_html[i]
        if c == '\r' or c == '\n':
            if c == '\r' and i + 1 < len(input_html) and input_html[i + 1] == '\n':
                i += 1
            current_line += 1
            current_column = 1
        else:
            if ord(c) > 0xFFFF:
                surrogate_pairs += 1
                current_column += 2
            elif (0xD800 <= ord(c) <= 0xDBFF and i + 1 < len(input_html)
                    and 0xDC00 <= ord(input_html[i + 1]) <= 0xDFFF):
                surrogate_pairs += 1
                i += 1
                current_column += 2
            else:
                current_column += 1
        i += 1
    return column - surrogate_pairs


def format_expected_error_assertions(errors: List, input_html: str) -> str:
    lines = [f"  @debug.assert_eq(errors.length(), {len(errors)})"]
    for i, error in enumerate(errors):
        variant = error_code_variant(error["code"])
        if "line" not in error or "col" not in error:
            raise ValueError(f"Tokenizer error has no source position: {error}")
        column = moonbit_error_column(input_html, error["line"], error["col"])
        lines.extend([
            f"  assert_true(errors[{i}].code is {variant})",
            f"  @debug.assert_eq(errors[{i}].position.line, {error['line']})",
            f"  @debug.assert_eq(errors[{i}].position.column, {column})",
        ])
    return '\n'.join(lines)
...
def generate_tokenizer_test(test: Dict[str, Any], index: int) -> Optional[str]:
    ...
    return f'''///|
test "{test_name}" {{
  let (tokens, errors) = @html.tokenize("{escaped_input}")
